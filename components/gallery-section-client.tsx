...
export function GallerySectionClient({ artworks }: { artworks: Artwork[] }) {
  return (
    <section id="gallery" className="py-24 lg:py-32 bg-background">
      <div className="container mx-auto px-6 lg:px-12">
        ...
        >
          ...
          <h2 className="text-4xl md:text-5xl lg:text-6xl font-serif mb-6">Curated Artworks</h2>
          <p className="max-w-2xl mx-auto text-muted-foreground leading-relaxed">
            Explore our handpicked selection of extraordinary pieces from emerging and established artists worldwide.
          </p>
        </motion.div>
              ...
              <Link href={`/artworks/${artwork.id}`}>
                <div className="relative aspect-[4/5] overflow-hidden">
                  ...
                  <div className="absolute inset-0 p-6 flex flex-col justify-end opacity-0 group-hover:opacity-100 transition-opacity duration-500">
                    <span className="text-xs tracking-[0.2em] uppercase text-accent mb-2">{artwork.category}</span>
                    <h3 className="text-xl font-serif text-foreground mb-1">{artwork.title}</h3>
                    <p className="text-sm text-muted-foreground mb-3">by {artwork.artist}</p>
                    <div className="flex items-center justify-between">
                      <span className="text-lg font-medium text-foreground">{artwork.price}</span>
                      <motion.span
